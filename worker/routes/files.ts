...
function cleanName(value: string): string {
  ...
  return cleaned.slice(0, 160) || 'material.pdf';
}

export async function uploadFileRoute(
  request: Request,
  env: Env,
): Promise<Response> {
  ...
  const originalName = cleanName(
    url.searchParams.get('name') ?? 'material.pdf',
  );
  ...
  const ownership = await restFetch(
    env,
    `/rooms?id=eq.${encodeURIComponent(roomId)}&select=id`,
    member.accessToken,
  );
  ...
  const objectPath =
    `${member.user.id}/${roomId}/${crypto.randomUUID()}.pdf`;

  const upload = await storageFetch(
    env,
    `/object/study-materials/${objectPath}`,
    member.accessToken,
    {
      method: 'POST',
      headers: {
  ...
  const saved = await restFetch(
    env,
    '/materials?select=id,original_name,size_bytes,status,created_at',
    member.accessToken,
    {
      method: 'POST',
      headers: {
